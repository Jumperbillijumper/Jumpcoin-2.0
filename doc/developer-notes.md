...
- [Development guidelines](#development-guidelines)
    - [General Jumpcoin](#general-bitcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Jumpcoin uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
Jumpcoin is a multi-threaded application, and deadlocks or other
...
Jumpcoin can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Jumpcoin, where everyone uses
...
pay attention to for reviewers of Jumpcoin code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Jumpcoin uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of Jumpcoin, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against Jumpcoin so that they can be integrated
...
file descriptors, it will cause problems with Jumpcoin's `select()` loop, because
...
compatibility between nodes. This happened in Jumpcoin 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Jumpcoin's
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Jumpcoin, the wallet was always
